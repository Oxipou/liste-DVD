...
Ce dépôt GitHub contient la liste des DVD familiaux.
...
Il y a moult façons de consulter la liste de DVD. Premièrement, il est simplement possible de regarder la version imprimée, qui est dans le classeur un peu transparent. Mais comme le classement peut varier selon les choix (sans compter que des films ont différents titres possibles en plusieurs langues), la liste est aussi disponible sur ce dépôt GitHub, [ici](./listes-DVD/liste-DVD.md#liste-dvd).

Si vous consultez la version numérique, il y a plusieurs petites choses qui pourraient vous faciliter la recherche :
- Même si c'est plutôt connu, je rappelle que *⌘ + F* (ou *ctrl + F* sur Windows/Linux) permet, sur ordinateur, de rechercher un ou des mots dans la liste. Pour les téléphones voici une vidéo pour faire la même chose sur iOS (Safari) [ici](./assets/tutos/recherche-sur-telephone/iphone.mp4) et une autre pour Android (Google Chrome) [ici](./assets/tutos/recherche-sur-telephone/android.mp4) (la première vidéo montre plein d'exemples d'utilisation alors que la deuxième ne montre que comment utiliser l'outil recherche).
- Puis j'ai mis des mots clés entre crochets et en italiques après certains titres, qui permettent aussi de faire des recherches comment ça (par exemple il y a *[Pixar]*, *[Charlie Chaplin]* ou *[Hayao Miyazaki]*).

Sinon pour les enfants il y a aussi le classeur avec les pochettes de DVD.

Et pour finir la simple méthode de directement regarder dans les tiroirs à DVD, qui sont aussi rangés dans le même ordre… mais c'est probablement la solution la plus fastidieuse.
